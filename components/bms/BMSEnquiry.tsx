...
const COURSES = [
  '46400 — Diploma in BMS Technician (6 Months)',
  '46401 — Certified BMS Engineer (3 Months)',
  '46402 — Certified ELV Engineer (3 Months)',
]

export default function BMSEnquiry() {
  ...
  function handleSubmit(ev: React.FormEvent) {
    ...
    const msg = encodeURIComponent(
      `Hi, I want to enquire about the BMS course at SMECLabs Kochi.\n\nName: ${form.name}\nPhone: ${form.phone}\nEmail: ${form.email || 'N/A'}\nCourse: ${form.course}`
    )
  ...
  return (
    <section id="contact" className="py-20 relative overflow-hidden"
      style={{ background: '#0F172A', backgroundImage: `radial-gradient(ellipse at 50% 50%, rgba(30,41,59,0.8) 0%, transparent 60%)` }}
    >
      ...
      <div className="max-w-xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-10">
          ...
          <h2 className="text-3xl sm:text-4xl font-black text-white">
            Enquire{' '}
            <span style={{ color: blue, textShadow: '0 0 28px rgba(59,130,246,0.5)' }}>Now</span>
          </h2>
        ...
        {submitted ? (
          <div className="rounded-2xl border p-8 text-center"
            style={{ background: 'rgba(30,41,59,0.6)', borderColor: 'rgba(59,130,246,0.3)' }}
          >
            <Icon icon="mdi:check-circle" className="w-12 h-12 mx-auto mb-4" style={{ color: '#22C55E' }} aria-hidden="true" />
            <h3 className="font-bold text-xl mb-2" style={{ color: '#E5E7EB' }}>Enquiry Sent!</h3>
          ...
          <form onSubmit={handleSubmit} noValidate
            ...
            style={{ background: 'rgba(30,41,59,0.4)', borderColor: border }}
          >
            {[
              { id: 'bms-name',  label: 'Full Name *',     type: 'text',  ac: 'name',  ph: 'Your full name',         key: 'name',  err: errors.name },
              { id: 'bms-phone', label: 'Mobile Number *', type: 'tel',   ac: 'tel',   ph: '10-digit mobile number', key: 'phone', err: errors.phone },
              { id: 'bms-email', label: 'Email (optional)',type: 'email', ac: 'email', ph: 'your@email.com',         key: 'email', err: undefined },
            ].map((f) => (
            ...
            <div className="flex flex-col gap-1.5">
              <label htmlFor="bms-course" className="text-xs font-bold uppercase tracking-widest" style={{ color: blue }}>
                Course *
              </label>
              <select id="bms-course" value={form.course}
                ...
                style={{
                  ...
                  color: form.course ? '#E5E7EB' : '#6B7280',
                }}
              >
                <option value="" disabled>Select a course</option>
                {COURSES.map((c) => (
                  <option key={c} value={c} style={{ background: '#1E293B', color: '#E5E7EB' }}>{c}</option>
                ))}
              </select>
            ...
            <button type="submit"
              className="flex items-center justify-center gap-2 w-full py-4 rounded-xl font-extrabold text-sm transition-colors bms-glow-pulse"
