...
const COURSES = [
  '47100 — Professional Diploma in Digital Marketing with AI (6 Months)',
]

export default function DigitalMarketingEnquiry() {
  ...
  function handleSubmit(ev: React.FormEvent) {
    ...
    const msg = encodeURIComponent(
      `Hi, I want to enquire about the Digital Marketing course at SMECLabs Kochi.\n\nName: ${form.name}\nPhone: ${form.phone}\nEmail: ${form.email || 'N/A'}\nCourse: ${form.course}`
    )
      ...
      <div className="max-w-xl mx-auto px-4 sm:px-6 relative z-10">
        <div className="text-center mb-10">
          ...
          <h2 className="text-3xl sm:text-4xl font-black text-white">
            Enquire{' '}
            <span className="dm-gradient-text">Now</span>
          </h2>
        ...
        {submitted ? (
          <div className="rounded-2xl border p-8 text-center"
            style={{ background: 'rgba(17,17,17,0.8)', borderColor: 'rgba(236,72,153,0.3)' }}>
            <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4"
              style={{ background: 'linear-gradient(135deg, #F97316, #EC4899, #8B5CF6)' }}>
              <Icon icon="mdi:check" className="w-7 h-7 text-white" aria-hidden="true" />
            </div>
          ...
          <form onSubmit={handleSubmit} noValidate
            ...
            style={{ background: 'rgba(17,17,17,0.6)', borderColor: 'rgba(236,72,153,0.18)' }}>

            {/* Gradient top bar */}
            <div className="-mx-6 sm:-mx-8 -mt-6 sm:-mt-8 mb-2 h-0.5 rounded-t-2xl"
              style={{ background: 'linear-gradient(90deg, #F97316, #EC4899, #8B5CF6)' }} />

            {[
              { id: 'dm-name',  label: 'Full Name *',     type: 'text',  ac: 'name',  ph: 'Your full name',         key: 'name',  err: errors.name },
              { id: 'dm-phone', label: 'Mobile Number *', type: 'tel',   ac: 'tel',   ph: '10-digit mobile number', key: 'phone', err: errors.phone },
              { id: 'dm-email', label: 'Email (optional)', type: 'email', ac: 'email', ph: 'your@email.com',        key: 'email', err: undefined },
            ].map((f, i) => {
              const labelColor = i % 3 === 0 ? '#F97316' : i % 3 === 1 ? '#EC4899' : '#8B5CF6'
              return (
                <div key={f.id} className="flex flex-col gap-1.5">
                  <label htmlFor={f.id} className="text-xs font-bold uppercase tracking-widest" style={{ color: labelColor }}>
                    {f.label}
                  </label>
                  <input id={f.id} type={f.type} autoComplete={f.ac} placeholder={f.ph}
                    ...
                    style={{ background: 'rgba(0,0,0,0.6)', borderColor: f.err ? '#EF4444' : 'rgba(236,72,153,0.2)' }}
                  />
            ...
            <div className="flex flex-col gap-1.5">
              <label htmlFor="dm-course" className="text-xs font-bold uppercase tracking-widest" style={{ color: '#8B5CF6' }}>
                Course *
              </label>
              <select id="dm-course" value={form.course}
            ...
            <button type="submit"
              className="flex items-center justify-center gap-2 w-full py-4 rounded-xl font-extrabold text-sm transition-opacity dm-glow-pulse"
